...
def initAllThings():
    ...
    own.position = [launchPos[0],launchPos[1],launchPos[2]+1]
...
def main():
    ...
    maxSpeed = g['rpm']/30#29.7230769
    maxThrust = g['thrust']*100
    lv = own.getLinearVelocity(True)
    currentSpeed = abs(lv[2])
    propThrottleCurve = .375
    thrust = (throttlePercent*.85)*(maxThrust-((currentSpeed**propThrottleCurve)/((maxSpeed**propThrottleCurve)/maxThrust)))
    if(thrust<1):
        thrust = 1
    ...
    if(own['oporational']):
        ...
        if(armed):
            ...
            #print(thrust)
            lvl = own.getLinearVelocity(False)
            ...
            if(propRay.positive==False):
                ...
                qd = [0.013014*dm,0.0111121*dm,0.0071081*dm] #air dampening
